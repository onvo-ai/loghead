...
function injectScript(config) {
  ...
  chrome.runtime.sendMessage({ type: 'INJECT_LOGGERHEAD' });

  // Listen for messages from injected script
  window.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'LOGGERHEAD_LOG') {
...
function sendToServer(payload, config) {
  ...
  chrome.runtime.sendMessage({
    ...
    if (!response || !response.success) {
      ...
      // console.error("[Loggerhead] Send failed:", response ? response.error : "Unknown error");
    }
  });
}
